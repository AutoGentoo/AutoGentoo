...
cdef DynamicBuffer DynamicBuffer_new_from_initial(void* initial, size_t size):
	self = DynamicBuffer()
	free(self.ptr)
...
cdef class Crypt:
	...
	cdef send_public(self, Socket s):
		...
		cdef char* send_pub = <char*>malloc(send_len + 1);
		BIO_write(public_bio_send, send_pub, <int>send_len)
		
		s.c_send(&send_len, sizeof(size_t), False)
		s.c_send(send_pub, send_len, False)
	...
	cdef recv_public(self, Socket s):
		...
		recv_buffer = <char*>malloc (recv_size + 1)
	...
	cdef rsa_t rsa_verify(self, Socket s):
		cdef rsa_t rsa_response = <rsa_t>-1;
		cdef int response = 0
		...
		cdef DynamicBuffer decrypted_temp = self.decrypt(DynamicBuffer_new_from_initial(encrypted, incoming_size))
		free(encrypted)
		if strcmp(<char*>decrypted_temp.ptr, "AutoGentooVerify") == 0:
			...
			response |= <int>AUTOGENTOO_RSA_SERVERSIDE_PUBLIC
		...
		if self.server_public_key == NULL:
			...
			s.c_send(&arbitrary_len, sizeof(size_t), False)
			s.c_send(arbitrary_send, arbitrary_len, False)
		else:
			s.c_send(&decrypted_send.n, sizeof(int), False)
