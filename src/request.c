...
Request* request_handle(Connection* conn) {
    ...
    while (current != STRCT_END) {
        if (current >= STRCT_MAX) AUTOGENTOO_REQUEST_HANDLE_VECTOR_ERROR
        ...
        if (len == -1) AUTOGENTOO_REQUEST_HANDLE_VECTOR_ERROR
        current_request += len;
...
void request_free(Request* req) {
    ...
    for (int i = 0; i != req->struct_c; i++) {
        request_structure_free(req->structures[i]->data,
                               request_structure_linkage[(request_structure_t) req->structures[i]->type - 1]);
