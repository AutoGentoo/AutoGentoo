...
int get_client_from_host (struct serve_client_manager manager, char* hostname) {
    ...
    for (i=0; i!=manager.used; i++) {
        if (strcmp(manager.clients[i].hostname, hostname) == 0) {
            return i;
        }
    }
...
void add_to_manager (struct serve_client_manager * manager, struct serve_client conf) {
    ...
    manager->clients[manager->used].manager = manager;*/
    manager->used++;
}
...
struct serve_client_manager init_manager (char * top_dir) {
    struct serve_client_manager out;
...
struct sserve_client_manager gen_simple_manager (struct serve_client_manager manager) {
    struct sserve_client_manager out;
    strncpy (out.top_dir, manager.top_dir, sizeof (out.top_dir) -1 );
    out.size = manager.size;
    out.used = manager.used;
    return out;
}

void write_manager (int fd, struct serve_client_manager manager) {
    struct sserve_client_manager simple_manager = gen_simple_manager (manager);
    write (fd, &simple_manager, sizeof (struct sserve_client_manager));
    
    int i;
    for (i=0; i != simple_manager.used; i++) {
        write (fd, &manager.clients[i], sizeof (struct serve_client));
    }
}

void read_manager (int fd, struct serve_client_manager * manager) { // You must malloc this pointer first
    struct sserve_client_manager s_buff;
    read (fd, &s_buff, sizeof(struct sserve_client_manager));
    
    strcpy(manager->top_dir, s_buff.top_dir);
    manager->size = s_buff.size;
    manager->used = s_buff.used;
    
    manager->clients = malloc (sizeof (struct serve_client) * manager->size);
    
    int buff_used = manager->used;
    manager->used = 0; // Used so that we can append the generated struct from the start of array
...
struct serve_client read_client (int fd) {
    struct serve_client r_client;
    
    read (fd, &r_client, sizeof(struct serve_client));
    return r_client;
}
