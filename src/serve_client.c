...
",  conf.config.CFLAGS, conf.config.CXXFLAGS, conf.config.CHOST,
    _ROOT_,
    ...
    char make_conf_file [128];
    sprintf (make_conf_file, "%s/autogentoo/etc/portage/make.conf");
    FILE * fp_mc;
    fp_mc = fopen (make_conf_file, "w+");
    fwrite (make_conf_buff, 1, sizeof (make_conf_buff), fp_mc);
    fclose (fp_mc);
}
