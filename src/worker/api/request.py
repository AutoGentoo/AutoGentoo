...
class Request:
    ...
    def __init__(self, adr: Address, request: int, structs: Union[List[RequestStruct], Tuple[RequestStruct]],
                 _ssl=True):
        self.address = adr
        self.ssl = _ssl
        ...
        for i, st in enumerate(self.structs):
            if st.struct_type != Request.ARGS[self.request][i]:
                raise TypeError(
                    "Invalid argument expected '%d' got '%d'" % (Request.ARGS[self.request][i], st.struct_type))
